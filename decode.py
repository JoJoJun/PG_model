...
"""This file contains code to run beam search decoding, including running ROUGE evaluation and producing JSON datafiles
for the in-browser attention visualizer, which can be found here https://github.com/abisee/attn_vis"""
...
import sys
import os
sys.path.append("/home/zzj/.local/lib/python3.5/site-packages")
...
class BeamSearchDecoder(object):
    ...
    def __init__(self, model, batcher, vocab):
        ...
        tf.logging.info("当前ckpt路径 --> %s", ckpt_path)
        if FLAGS.single_pass:
            ...
            tf.logging.info("当前decode路径 --> %s", self._decode_dir)
    ...
    def decode(self):
        ...
        while True:
            ...
            if FLAGS.single_pass:
                print_results(article_withunks, abstract_withunks, decoded_output, counter)  # log output to screen
                if FLAGS.decode_rouge:
    ...
    def write_for_bleu(self, reference_sents, decoded_words):
        ...
        """
        # First, divide decoded output into sentences
        reference_sentence = ' '.join(reference_sents)
...
def print_results(article, abstract, decoded_output, counter=None):
