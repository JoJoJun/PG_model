...
class SummarizationModel(object):
    ...
    def _add_placeholders(self):
        ...
        self._dec_batch = tf.placeholder(tf.int32, [hps.batch_size, hps.max_dec_steps],
                                         name='dec_batch')  # max timesteps of decoder (max summary tokens)
...
LSTMStateTuple由（c，h）组成，分别代表memory cell和hidden state。'''
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('embedding'):
                ...
                embedding = tf.get_variable('embedding', [vsize, hps.emb_dim], dtype=tf.float32,
                                            initializer=tf.constant_initializer(embedding_arr))
            ...
            with tf.variable_scope('output_projection'):
                ...
                # vocab_dists = [tf.nn.softmax(tf.clip_by_value(s,1e-8,tf.reduce_max(s))) for s in
                #                vocab_scores]
    ...
    def _add_train_op(self):
        ...
        tf.summary.scalar('learning_rate', learning_rate)  # 可视化lr的变化
