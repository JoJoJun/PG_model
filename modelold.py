...
class SummarizationModel(object):
    ...
    def _add_placeholders(self):
        ...
        # decoder part
        self._dec_batch = tf.placeholder(tf.int32, [hps.batch_size, hps.max_dec_steps], name='dec_batch')#max timesteps of decoder (max summary tokens)
        ...
        # self._embedding = tf.placeholder(tf.float32,[vsize, hps.emb_dim],name='embedding')
    ...
    def _make_feed_dict(self, batch, just_enc=False):
        ...
        # _, embd = self.loadWord2vec('vectorsW.txt')
    ...
    def _add_encoder(self, encoder_inputs, seq_len):
        ...
        with tf.variable_scope('encoder'):
            ...
            # they are concatenated along the column axis. This latter behavior will soon be deprecated
            #前向rnn
...
LSTMStateTuple由（c，h）组成，分别代表memory cell和hidden state。'''
    ...
    def _add_emb_vis(self, embedding_var):
        ...
        embedding.metadata_path = os.path.abspath(vocab_metadata_path)
        projector.visualize_embeddings(summary_writer, config)

    def loadWord2vec(self,filename,size):
        print('loaded word2vec')
        embedding = np.loadtxt(filename)
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            # Some initializers
            #生成具有均匀分布的张量的初始化器。  下限 上限 种子
            self.rand_unif_init = tf.random_uniform_initializer(-hps.rand_unif_init_mag, hps.rand_unif_init_mag,
                                                                seed=123)
            self.trunc_norm_init = tf.truncated_normal_initializer(stddev=hps.trunc_norm_init_std)#1e-4

            # Add embedding matrix (shared by the encoder and decoder inputs)
            with tf.variable_scope('embedding'):
                embedding_arr = self.loadWord2vec('emb.txt',vsize)
                # W = tf.get_variable('embedding',[vsize, hps.emb_dim],
                #                 trainable=False,initializer=tf.constant_initializer(embedding))

                embedding = tf.get_variable('embedding', [vsize, hps.emb_dim], dtype=tf.float32,initializer=tf.constant_initializer(embedding_arr))
            ...
            if hps.mode in ['train', 'eval']:
                ...
                with tf.variable_scope('loss'):
                    ...
                    tf.summary.scalar('loss', self._loss)#可视化loss的变化
    ...
    def _add_train_op(self):
        ...
        # Clip the gradients
        with tf.device("/gpu:" + str(FLAGS.device) if FLAGS.device != '' else "/cpu:0"):
            grads, global_norm = tf.clip_by_global_norm(gradients, self._hps.max_grad_norm)

        # Add a summary
        tf.summary.scalar('global_norm', global_norm)
        # 指数衰减
        starter_learning_rate = self._hps.lr
        global_step = self.global_step
        learning_rate = tf.train.exponential_decay(starter_learning_rate, global_step,
                                                   100000, 0.96, staircase=True)
        # self._hps.lr = learning_rate
        # self._hps = self._hps._replace(lr = learning_rate)

        # Apply adagrad optimizer
        # optimizer = tf.train.AdagradOptimizer(self._hps.lr, initial_accumulator_value=self._hps.adagrad_init_acc)


        optimizer = tf.train.AdagradOptimizer(learning_rate, initial_accumulator_value=self._hps.adagrad_init_acc)
    ...
    def build_graph(self):
        ...
        if self._hps.mode == 'train':
            self._add_train_op()
        self._summaries = tf.summary.merge_all()#将之前定义的summary op 整合到一起
        t1 = time.time()
        tf.logging.info('Time to build graph: %i seconds', t1 - t0)

    def run_train_step(self, sess, batch, metadata=False, summary_writer=None):
        ...
        if metadata and summary_writer is not None:
            run_options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE)#使用tf.RunOption定义Tensorflow运行选项
            run_metadata = tf.RunMetadata()#运行的元信息
            ret = sess.run(to_return, feed_dict, run_metadata=run_metadata, options=run_options)
            summary_writer.add_run_metadata(run_metadata, 'step%d' % ret['global_step'], global_step=ret['global_step'])
            return ret
        else:
